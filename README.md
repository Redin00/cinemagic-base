...
| -------------------- | ---------------------------------- | ---------------------------------------------- |
| `SC_DOMAIN`          | `streamingcommunityz.taxi`         | Upstream catalogue domain (rotates often)      |
| `SC_PORT`            | `8000`                             | Local port for the Python service              |
| `SC_VIXSRC_DOMAIN`   | `vixsrc.to`                        | Playback host (rotates often)                  |
...
1. `https://<SC_VIXSRC_DOMAIN>/api/{movie,tv}/<tmdbId>[/<season>/<episode>]`
...
- films — `https://<SC_VIXSRC_DOMAIN>/movie/<tmdbId>`
- series — `https://<SC_VIXSRC_DOMAIN>/tv/<tmdbId>/<season>/<episode>`

Set `SC_VIXSRC_DOMAIN` to an empty value to disable playback entirely; the Python
service reports the active host at `GET /player`.
